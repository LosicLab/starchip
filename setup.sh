...
if [ "$#" -ne 3 ]; then
    echo "STARChip Setup Tool usage: setup.sh /path/to/gtf.file /path/to/genome.fasta.file /path/to/desired/output/directory/"
    exit 1
fi
filename=`basename $1`
...
${DIR}/scripts/setup/fasta_getlength.pl $2 | sort -k1,1 > ${3}${filename}.genome 
