...
my $cnvfile = $script_dir . 'data/conrad_' . $Configs{genome} . '.cnvs'; 
		...
		if ($Configs{pairedend} eq "TRUE") {  
			for my $key2 (keys %fusions) { #cycle through all the keys again
				...
				if ($fusions{$key2}[2][0] >= 1 || $fusions{$key2}[2][5] >=1 ) { #we are only really interested in sites with spanning fusions
					...
					next if ($key2array[0] ~~$key2array[2] && (abs($key2array[1]-$key2array[3])<=$Configs{samechrom_wiggle}) ); #skip same chrom proximal
					#check if the fusion from keyarray (has jxn crossing) has the same coordinates (within $Configs{wiggle} bp) as $keyarray2
					if ($keyarray[4]~~$key2array[4] && $keyarray[5]~~$key2array[5] && $keyarray[0]~~$key2array[0] && $keyarray[2]~~$key2array[2] && (abs($keyarray[1]-$key2array[1])<=$Configs{wiggle}) && (abs($keyarray[3]-$key2array[3])<=$Configs{wiggle}) ) {
					...
					elsif (&reversestrand($keyarray[4])~~$key2array[5] && &reversestrand($keyarray[5])~~$key2array[4] && $keyarray[0]~~$key2array[2] && $keyarray[2]~~$key2array[0] && (abs($keyarray[1]-$key2array[3])<=$Configs{wiggle}) && (abs($keyarray[3]-$key2array[1])<=$Configs{wiggle}) ) {
